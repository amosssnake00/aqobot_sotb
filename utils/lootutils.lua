...
local validActions = { keep = 'Keep', bank = 'Bank', sell = 'Sell', ignore = 'Ignore', destroy = 'Destroy', quest =
'Quest', tribute = 'Tribute' }
...
local NEVER_SELL = { ['Diamond Coin'] = true, ['Celestial Crest'] = true, ['Gold Coin'] = true, ['Taelosian Symbols'] = true,
    ['Planar Symbols'] = true }
local beeploots = { ['Upper Runic Fragment'] = true, ['Center Runic Fragment'] = true, ['Lower Runic Fragment'] = true,
    ['A Placid Void'] = true, ['A Radiant Morsel'] = true }
...
local function getRule(item)
    ...
    if loot.AlwaysEval then
        ...
        if (oldDecision == 'Sell' and stackable) and (sellPrice * stackSize >= loot.StackPlatValue) then resetDecision =
            oldDecision end
    ...
    if lootData[firstLetter][itemName] == 'NULL' then
        ...
        if (stackable and loot.StackPlatValue > 0) and (sellPrice * stackSize < loot.StackPlatValue) then lootDecision =
            'Ignore' end
        ...
        if tributeValue >= loot.MinTributeValue and (sellPrice < loot.MinSellPrice or sellPrice == 0) then lootDecision =
            'Tribute' end
    ...
    if mq.TLO.Window('LootWnd').Open() and items > 0 then
        ...
        for i = 1, items do
            ...
            if corpseItem() then
                ...
                if beeploots[corpseItem.Name()] or (mq.TLO.Zone.ShortName() == 'sebilis' and (corpseItem.Name():find('Forsaken') or corpseItem.Name():find('Bloodstained'))) then
                    mq.cmdf('/dga /pop 5 %s', corpseItem.Name()) end
