...
function Cleric:init()
    ...
    self.classOrder = { 'heal', 'cure', 'rez', 'aggro', 'assist', 'debuff', 'burn', 'mash', 'cast', 'recover', 'buff',
        'rest' }
    self:initBase('CLR')
...
function Cleric:initClassOptions()
    ...
    if not state.emu then self:addOption('USESPLASH', 'Use Splash', true, nil,
            'Toggle use of splash heal + twincast nuke', 'checkbox', nil, 'UseSplash', 'bool') end
    self:addOption('USEVIE', 'Use Vie', true, nil, 'Toggle use of Vie spell line', 'checkbox', nil, 'UseVie', 'bool')
    self:addOption('USEHAMMER', 'Use Hammer', false, nil, 'Toggle use of summoned hammer pet', 'checkbox', nil,
        'UseHammer', 'bool')
    -- HoTs only mem'd in BYOS
    self:addOption('USEHOTGROUP', 'Use Group HoT', true, nil, 'Toggle use of group HoT', 'checkbox', nil, 'UseHoTGroup',
        'bool')
    ...
    self:addOption('USEDEBUFF', 'Use Reverse DS', true, nil, 'Toggle use of Mark reverse DS', 'checkbox', nil,
        'UseDebuff', 'bool')
    self:addOption('USESYMBOL', 'Use Symbol', false, nil, 'Toggle use of Symbol buff line', 'checkbox', nil, 'UseSymbol',
        'bool')
    self:addOption('USERETORT', 'Use Retort', true, nil, 'Toggle use of Retort spell line', 'checkbox', nil, 'UseRetort',
        'bool')
...
Cleric.SpellLines = {
    ...
    -- Buffs
    { Group = 'aura',     Spells = { 'Bastion of Divinity', 'Aura of Divinity' },                                                                                                                                                         Options = { aura = true, aurabuff = true, condition = function() return not
        state.emu or not mq.TLO.Me.AltAbility('Spirit Mastery')() end } },
    { Group = 'spellhaste', Spells = { 'Hand of Devotion', 'Hand of Devoutness', 'Hand of Reverence', 'Hand of Sanctity', 'Hand of Zeal', 'Hand of Will', --[[emu cutoff]] 'Aura of Devotion', 'Blessing of Devotion', 'Blessing of Reverence'  }, Options = { selfbuff = true, classes = { CLR = true, DRU = true, SHM = true, MAG = true, ENC = true, WIZ = true, NEC = true }, alias = 'SPELLHASTE', condition = function() return
    ...
    },
    {
        ...
        Options = { opt = 'USESYMBOL', classes = { CLR = true, DRU = true, SHM = true, MAG = true, ENC = true, WIZ = true, NEC = true }, condition = function() return
            mq.TLO.SpawnCount('pc group class druid')() > 0 end, alias = 'SYMBOL' }
    },
    {
        Group = 'singlesymbol',
        ...
        Options = { opt = 'USESYMBOL', classes = { CLR = true, DRU = true, SHM = true, MAG = true, ENC = true, WIZ = true, NEC = true }, condition = function() return
            mq.TLO.SpawnCount('pc group class druid')() > 0 end, alias = 'SINGLESYMBOL' }
    },
    ...
    { Group = 'bigvie', Spells = { 'Rallied Greater Aegis of Vie', 'Rallied Greater Blessing of Vie', 'Rallied Greater Protection of Vie', 'Rallied Greater Guard of Vie', 'Rallied Greater Ward of Vie', --[[emu cutoff]] 'Panoply of Vie' }, Options = { opt = 'USEVIE', alias = 'VIE', selfbuff = true, Gem = function(
        lvl) return lvl <= 70 and 4 or nil end } },
    ...
    { Group = 'hammerpet', Spells = { 'Unswerving Hammer of Justice' },                                                                                                                                                            Options = { Gem = function(
        lvl) return lvl <= 70 and not Cleric:isEnabled('USESTUN') and 11 or nil end, opt = 'USEHAMMER', precast = function()
        mq.cmdf('/mqt id %s', state.assistMobID)
        mq.delay(1)
    end } },
    { Group = 'rgc',     Spells = { 'Remove Greater Curse' },                                                                                                                                                                      Options = { cure = true, Curse = true, Gem = function(
        lvl) return lvl <= 70 and 12 or nil end } },
    ...
    { Group = 'nuke',    Spells = { 'Reproach', 'Ancient: Pious Conscience' },                                                                                                                                                     Options = { opt = 'USENUKES', Gem = function(
        lvl) return lvl <= 70 and 10 or nil end } },
    { Group = 'rez',     Spells = { 'Reviviscence' },                                                                                                                                                                              Options = { rez = true } },
}

Cleric.compositeNames = { ['Ecliptic Blessing'] = true, ['Composite Blessing'] = true, ['Dissident Blessing'] = true,
    ['Undying Life'] = true }
...
Cleric.Abilities = {
    ...
    },
    {
        ...
        Options = { fade = true, opt = 'USEFADE', precast = function()
            mq.TLO.Me.DoTarget()
            mq.delay(100)
        end, postcast = function()
            mq.delay(1000)
            mq.cmd('/makemevis')
        end }
    },
    ...
    },
    {
        Type = 'AA',
        Name = 'Turn Undead',
        Options = { dps = true, condition = function() return (mq.TLO.Target.PctHPs() or 100) < 95 and
...
function Cleric:initHeals()
    ...
    table.insert(self.healAbilities,
        common.getItem('Weighted Hammer of Conviction', { tank = true, regular = true, panic = true, pet = 60 }))
