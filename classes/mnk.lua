...
function Monk:initClassOptions()
    self:addOption('USEFD', 'Use Feign Death', true, nil, 'Toggle Use of Feign Death to reduce aggro', 'checkbox', nil,
        'UseFD', 'bool')
    self:addOption('USEFADE', 'Use Imitate Death', true, nil, 'Toggle use of Imitate Death in combat', 'checkbox', nil,
        'UseFade', 'bool')
    self:addOption('USEIMPENETRABLE', 'Use Impenetrable', true, nil, 'Toggle use of Impenetrable Discipline in combat',
        'checkbox', nil, 'UseFade', 'bool')
end
...
Monk.Abilities = {
    ...
    { -- if another monks has faded
        ...
        Options = { second = true, condition = function() return not Monk.speedfocus or
            not mq.TLO.Me.CombatAbilityReady(Monk.speedfocus.Name)() end }
    },
    ...
    -- 3rd burn
    {
        ...
        Options = { third = true, condition = function() return (not Monk.palm or not mq.TLO.Me.CombatAbilityReady(Monk.palm.Name)()) and
            (not Monk.speedfocus or not mq.TLO.Me.CombatAbilityReady(Monk.speedfocus.Name)()) end }
    },
    { -- inc dmg, inc min dmg
        Type = 'Disc',
        Group = 'eyeofthestorm',
    ...
    -- Buffs
    {
        ...
        Options = { aurabuff = true, CheckFor = 'Disciples Aura', condition = function() return not mq.TLO.Me.Song(
            'Master\'s Aura')() and not mq.TLO.Me.Song('Disciple\'s Aura')() end }
    },
    {
        Type = 'Item',
    ...
    },
    {
        Type = 'Skill',
        Name = 'Feign Death',
        Options = { aggroreducer = true, opt = 'USEFD', postcast = function()
            mq.delay(500)
            mq.cmd('/stand')
            mq.cmd('/makemevis')
        end }
