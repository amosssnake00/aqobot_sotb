...
local state = {
    ...
    actors = {},
    debug = false,
    paused = false, -- on emu, start with guns blazing
    burnNow = false,
    burnActive = false,
    ...
    resists = {},
    medding = false,
    swapGem = mq.TLO.Me.NumGems() or 8, -- Gem used for temporary spell swaps, now defaults to the last available gem.
...
function state.handleCastingState(class)
    if state.casting then
        ...
        if not mq.TLO.Me.Casting() then
            ...
            if state.casting and state.casting.clickyType and mq.TLO.Me.ItemReady(state.casting.name)() and state.casting.timer then
                state.casting.timer:reset(0) end
